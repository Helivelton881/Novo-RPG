...
Numérica (Node, sem navegador): `test/dungeon-camera.test.js` prova que spawn/boss/exitPoint da masmorra sempre ficam dentro do viewport da câmera em 6 combinações de tela × 3 níveis de zoom, mesmo com foco de UI extremo, e que `applyDungeonState` chama `resize()` na ordem certa. `test/dungeon-visual.test.js` confirma que `DUNGEON_THEME_KIT` cobre exatamente as 7 zonas de `MASMORRA_CFG` (nunca diverge), que `kitProp` está protegido por `try/catch`, e que `ensureFieldSprites` é chamado antes de qualquer desenho.

Ao vivo no navegador (servidor local, `window.__G.buildMasmorra(tema,seed,[])` — bypassa autenticação real, testa só o pipeline de renderização/colisão): as 7 zonas constroem sem erro (43–47 props cada); nenhum dos 12 vãos de porta de `DUNGEON_CONNECTIONS_V2` fica bloqueado em nenhum tema; o ponto de spawn fica livre nas 7 zonas mesmo com a decoração nova; viewport mobile (375×812) renderiza o jogador imediatamente visível, sem o bug relatado. Screenshots confirmaram visualmente: entrada com tochas, encruzilhada com pilar/entulho/túmulo, sala de elite com sarcófagos flanqueando a passagem, arena do chefe com altar e baú, tema vulcânico com poças de lava e rocha vulcânica distintos do tema críptico.

`npm test` permanece verde (566 testes / 389 passando / 0 falhando / 177 pulados) — nenhuma regressão em nenhum sistema (`server.js` não foi tocado nesta fase).
