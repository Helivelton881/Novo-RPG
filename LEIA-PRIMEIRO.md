...
- **Ouro/gema/item por abate** continuam 100% client-side — é um mecanismo diferente (o abate solta um item no chão via `dropLoot`/`dropItem`, o jogador anda até ele e só aí `pickup()` credita), não um crédito direto no momento da morte. Fica pra uma unidade futura.
- **Sequência de missão via contagem de abate** (`P.kills`/`P.gk`, que avançam `P.quest` dentro de `killMob()`) ainda não é validada contra os contadores agora reais — um save forjado ainda pode setar `quest` adiante via `PUT` genérico e reivindicar a recompensa de missão legitimamente pelo endpoint da unidade 1, sem ter matado nada. Fechar isso exigiria validar o grafo inteiro de progressão de missão contra abates reais, escopo maior que esta unidade.
- **Sequitos temporários** (`temp:true` no manifesto — nascem `dead:true`, só "revivem" quando o chefe correspondente entra num estado de invocação, uma mecânica de IA que não existe no servidor) nunca chegam a ser dano-áveis via `mob_damage` hoje — ficam no manifesto por completude, mas não geram crédito na prática. Não é uma regressão (nunca foram rastreados antes) nem uma brecha (o jogador só deixa de ganhar o crédito do servidor pra esses abates específicos, cai no cálculo local).
