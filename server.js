...
// Fase 5.16.4 (bug real de producao, achado com log ao vivo -- IA de vila
// medindo "combat"/"hunt" contra targetMobId tipo "vila:7"): a vila TEM
// mobs de verdade (slimes fracos, level 1-3, populados sob demanda em
// map_join quando um jogador humano visita -- ver o branch `map==='vila'`
// dentro do handler de map_join). O guard contra a IA social entrar em
// combate so cobria a transicao 'travel' (comentario original abaixo) --
// nunca o proprio gatilho de hunt, que le state.mobs.size igual a IA de
// campo faria. Resultado: assim que algum humano visitava a vila e os
// slimes apareciam, a IA social passava a canitar/lutar contra eles,
// parada atacando por 8-23s reais repetidas vezes (medido ao vivo) -- e
// isso, nao o dist<4 ja corrigido antes, era o "andando travado" que
// sobrou apos aquele fix. `ai.map!=='vila'` aqui e a MESMA garantia que
// o guard de travel ja fazia, so que tambem no ponto onde ela realmente
// era violada.
function aiDoIdle(ai, now) {
  const state = ai.map !== 'vila' ? maps.get(ai.map) : null;
  if (state && state.mobs.size) {
...
function aiDoWander(ai, now) {
  ...
  const state = ai.map !== 'vila' ? maps.get(ai.map) : null; // mesma garantia de aiDoIdle -- nunca hunt pra IA social da vila
